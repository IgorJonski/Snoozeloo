...
- **iOS** (`AlarmKitAlarmScheduler`): `sync` schedules the AlarmKit alarm with `.relative(AlarmTime, repeats = .weekly(RepeatDays) | .never)` under the Alarm's main AlarmKit id. If `snoozedUntil` is ahead of now, it additionally schedules a second AlarmKit alarm under the Alarm's snooze id with `.fixed(snoozedUntil)`, the same attributes and `postAlert: 300` (so it can be snoozed again). `snooze` called from the in-app Trigger maps to `AlarmManager.countdown(id:)`; a Snooze tapped on the system alert is performed by AlarmKit itself and reported to the app through the secondary `LiveActivityIntent`, which records `snoozedUntil`. The bridge shape is #12.
...
6. **What cancels a pending Snoozed Occurrence**: any change to a *scheduling* field (Alarm Time, Repeat Days, Enabled) or deletion. Changes to *presentation* fields (Name, Ringtone, Volume, Vibrate) keep it, and it rings with the new settings on both platforms (Android reads the Alarm when it fires; iOS re-creates the `.fixed` snooze alarm on `sync`). Undo-delete restores the Alarm without its Snoozed Occurrence.
...
3. For each Disabled Alarm, ensure the platform holds nothing (both ids on iOS, both request codes on Android).
...
It is idempotent, so it is called without checking whether anything drifted: on every app start on both platforms; on Android also from `BOOT_COMPLETED`, `MY_PACKAGE_REPLACED`, `TIMEZONE_CHANGED`, `TIME_CHANGED` and `ACTION_SCHEDULE_EXACT_ALARM_PERMISSION_STATE_CHANGED` (receiver declarations: #11); on iOS also when the app returns to the foreground (bridge query of `AlarmManager.alarms`: #12).
...
- **Presentation edits during snooze cancel it, or on iOS ring with the old settings** — rejected once `.fixed(snoozedUntil)` proved to re-create the snooze with new settings; the only cost is that the native countdown Live Activity disappears after such an edit.
...
- On iOS every Alarm maps to two AlarmKit ids (main and snooze), derived deterministically from `AlarmId`; `syncAll()` must clean both. After a presentation edit during snooze, the Dynamic Island countdown is gone (`.fixed` without `preAlert` shows none) although the ring time is unchanged.

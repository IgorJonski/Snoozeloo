...
| Alarm ringtone | `SettingRow("Alarm ringtone", onClick, trailing = ringtoneName)`; empty while unresolved | always |
...
- **Ringtone name**: `GetRingtone(id)` (`catalog.find(id) ?: catalog.find(Default)`) on load and after every change of the draft's id; the result's `displayName` fills the row. On failure the row value is empty and the row stays tappable — no name we have not confirmed is shown.
...
data class AlarmSettingsState(
    ...
    val ringtoneName: String? = null,        // null → empty row value
    val volume: Int = 50,
